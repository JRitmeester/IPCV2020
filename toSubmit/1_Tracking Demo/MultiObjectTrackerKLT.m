...
%   Points       - tracked points from all ROI
%   PointIds     - ids of ROI associated with each point
...
%   addDetections - add ROI bounding boxes
%   track         - track the objects
%   Private methods:
%   RANSACing     - find and eleminate outliers and estimate (x,y)
%   repopulate    - generate random points inside ROI
        ...
        function EstimatedXY = RANSACing(this)
            ...
            for i = ROInum
                ...
                if size(pointsIn,2) >= 11
                    ...
                    if (mod(this.counter, 10) == 0)
                        if size(validPoints,1)<40 && size(validPoints,1) >5
